...
# Evaluation of an individual for each data point
    ...
    if points.shape[1] == 1:
        ...
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6])) for pos in range(points.shape[0])]
    elif points.shape[1] == 8:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7])) for pos in range(points.shape[0])]
    elif points.shape[1] == 9:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7], points[pos][8])) for pos in range(points.shape[0])]
    elif points.shape[1] == 10:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7], points[pos][8], points[pos][9])) for pos in range(points.shape[0])]
    elif points.shape[1] == 11:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7], points[pos][8], points[pos][9], points[pos][10])) for pos in range(points.shape[0])]
    elif points.shape[1] == 12:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7], points[pos][8], points[pos][9], points[pos][10], points[pos][11]))
              for pos in range(points.shape[0])]
    elif points.shape[1] == 13:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7], points[pos][8], points[pos][9], points[pos][10], points[pos][11]
                    , points[pos][12])) for pos in range(points.shape[0])]
    elif points.shape[1] == 14:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7], points[pos][8], points[pos][9], points[pos][10], points[pos][11]
                    , points[pos][12], points[pos][13])) for pos in range(points.shape[0])]
    elif points.shape[1] == 15:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7], points[pos][8], points[pos][9], points[pos][10], points[pos][11]
                    , points[pos][12], points[pos][13], points[pos][14])) for pos in range(points.shape[0])]
    elif points.shape[1] == 16:
        fx = [(func(points[pos][0], points[pos][1], points[pos][2], points[pos][3], points[pos][4], points[pos][5],
                    points[pos][6], points[pos][7], points[pos][8], points[pos][9], points[pos][10], points[pos][11]
                    , points[pos][12], points[pos][13], points[pos][14], points[pos][15])) for pos in range(points.shape[0])]
...
def KP_run(x,y, ngen=1000, size= 8, ops={'add','mul'}, Deph=(2,6), Deph_max=8, y_max=1, tol=1e-10, \
 ...
 # Return:
    ...
    for n_var in range(x.shape[1]): 
        # aux_str= 'ARG'+str(n_var)
        # pset.renameArguments( aux_str ='X'+str(n_var))
        if n_var==0:
            ...
            pset.renameArguments( ARG10 ='X'+str(n_var))
        elif n_var==11:
            pset.renameArguments( ARG11 ='X'+str(n_var))
        elif n_var==12:
            pset.renameArguments( ARG12 ='X'+str(n_var))
        elif n_var==13:
            pset.renameArguments( ARG13 ='X'+str(n_var))
        elif n_var==14:
            pset.renameArguments( ARG14 ='X'+str(n_var))
        elif n_var==15:
            pset.renameArguments( ARG15 ='X'+str(n_var))
        elif n_var==16:
            pset.renameArguments( ARG16 ='X'+str(n_var))
        elif n_var==17:
            pset.renameArguments( ARG17 ='X'+str(n_var))
        elif n_var==18:
            pset.renameArguments( ARG18 ='X'+str(n_var))
        elif n_var==19:
            pset.renameArguments( ARG19 ='X'+str(n_var))
        elif n_var==20:
            pset.renameArguments( ARG20 ='X'+str(n_var))
        elif n_var==21:
            pset.renameArguments( ARG21 ='X'+str(n_var))
        else:
    ...
    if y.shape[1]>1:
        return BestPop, BestQual, BestBeta, EcEst, RMSE_Best, RMSE_Best_Norm, logbook, mod_sim,I
    else:
